...
class MotorControl:
    ...
    def __init__(self, transport, servos):
        ...
        self.servos = servos
        arduino_port = detect_arduino_device()  
        if arduino_port is not None:
            self.arduino_serial = serial.Serial(arduino_port, 19200)  
        else:
    ...
    def omniwheel(self, W, Vx, Vy):
        ...
        matrix_A = np.array([1, np.tan(np.radians(gamma))]) /0.033
        ...
        matrix_C = np.array([[-0.035, 1, 0], [0.061, 0, 1]])
        matrix_C2 = np.array([[0.05, 1, 0], [0.05, 0, 1]])
        matrix_C3 = np.array([[0.05, 1, 0], [-0.05, 0, 1]])
        matrix_C4 = np.array([[-0.035, 1, 0], [-0.061, 0, 1]])
    ...
    async def execute_command(self, W, Vx, Vy, K, timeout):
        ...
        while time.time() -start_time < timeout:
            ...
            if K == 1 and self.arduino_serial is not None:  
    ...
    async def run_server(self):
        ...
        while True:
            try:
                ...
                W, Vx, Vy, K = map(float, message.split(','))
                print(f"Processing W: {W}, Vx: {Vx}, Vy: {Vy}, Kick: {K}")
                await self.motor_control.execute_command(W, Vx, Vy, K, 0.2)
