class agent:
	...
	def check_boundary(self, position):
		...
		field_width = 2760 #mm
		safety_border = 150 #mm
		if position[0] < 0:
			position[0] = max(-field_length/2+safety_border, position[0])
		else:
			position[0] = min(field_length-safety_border, position[0])

		if position[1] < 0:
			position[1] = max(-field_width+safety_border, position[1])
		else:
			position[1] = min(field_width-safety_border, position[1])
